          ...
          <div className="flex items-center gap-1">
            {[h, m, s].map((t, i) => (
              <span key={i} className="flex items-center gap-1">
                <span className="bg-[#EE4D2D] text-white font-black text-sm px-2 py-0.5 rounded-lg min-w-[28px] text-center">{t}</span>
                {i < 2 && <span className="text-[#EE4D2D] font-black">:</span>}
              </span>
            ))}
          </div>
        </div>
        <Link href="/products" className="text-[#EE4D2D] text-sm font-bold hover:underline">ທັງໝົດ &gt;</Link>
      </div>
        ...
        {products.map(p => {
          ...
          return (
            <Link key={p.id} href={`/products/${p.id}`}
              className="shrink-0 w-36 bg-[#1A2540] rounded-xl overflow-hidden hover:scale-105 transition-transform group">
              ...
              <div className="p-2 space-y-1">
                <p className="text-white text-xs line-clamp-1 font-medium">{p.name}</p>
                <p className="text-[#EE4D2D] font-black text-sm">{fmt(p.discount_price ?? p.price)}</p>
                {/* Sold bar */}
                <div>
                  <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full bg-[#EE4D2D] rounded-full" style={{ width: `${sold}%` }} />
                  </div>
                  <p className="text-white/50 text-[10px] mt-0.5">ລາຍ {sold}%</p>
                </div>
                <button onClick={e => handleAdd(e, p)}
                  className={`w-full py-1 rounded-lg text-xs font-bold transition-colors ${added === p.id ? 'bg-green-500 text-white' : 'bg-[#EE4D2D] text-white hover:bg-[#d63d1e]'}`}>
